...
import com.example.studentmanager.databinding.FragmentGradesScreenBinding
...
class GradesScreen : Fragment() {

    private var _binding: FragmentGradesScreenBinding? = null
    ...
    private var selectedGrades: String? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentGradesScreenBinding.inflate(layoutInflater, container, false)
        return binding.root
    }
    ...
    private fun setupMotherChipGroups(students: List<Student>) {
        val uniqueGrades = students.map { it.grade }.distinct().sorted()
        ...
        addMotherChipGroup("Grades", uniqueGrades) { selectedGrades = it; filterStudents() }
    ...
    private fun filterStudents() {
        val filtered = allStudents.filter { s ->
            (selectedGrades == null || s.grade == selectedGrades) &&
